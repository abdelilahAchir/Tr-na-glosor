    ...
    public partial class WordList
    {
        ...
        public static WordList LoadList(string name)
        {
            ...
            if (searchedList != "")
            {
                ...
                using (StreamReader reader = new StreamReader(path))
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(languagesString))
                    {
                        languagesString = languagesString.ToLower();
                    }
                    ...
                    while (!reader.EndOfStream)
                    {
                        ...
                        {
                            if (translationsString.Split(';').ToList().Count > 1)
                            {
                                translations.Add(translationsString.Split(';'));
                            }
                            else
                            {
                                MessageBox.Show("this file Contains Only one translation!" +
                                                "     Select a file with more than one translation!".ToUpper());
                                break;
                            }
                        }
                    }
        ...
        public bool Remove(int translation, string word)
        {
            ...
            foreach (var wrd in _words)
            {
                ...
                {
                    _words = _words.Where(w => w.Translations[translation].ToLower() != word.ToLower()).ToList();
                    removed = true;
                }
                else
                {
